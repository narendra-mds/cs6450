...
class NPNTanh(torch.nn.Module):
    def __init__(self):
        super(NPNTanh, self).__init__()
        self.xi_sq = math.pi / 8
        self.alpha = 4 - 2 * math.sqrt(2)
        self.beta = - math.log(math.sqrt(2) + 1)
    ...
    def forward(self, x):
        assert(len(x) == 2)
        o_m, o_s = x
        a_m = torch.tanh(o_m / (1 + self.xi_sq * o_s) ** 0.5)
        a_s = self.alpha * (1 - torch.tanh((o_m + self.beta) / (2 * self.xi_sq * self.alpha ** 2 * o_s) ** 0.5) ** 2) - a_m ** 2
        return a_m, a_s
