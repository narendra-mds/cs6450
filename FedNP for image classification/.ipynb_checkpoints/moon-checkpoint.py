...
parser.add_argument('--experiment', default='MOON', type=str)
...
prev_models = [ResNet18(num_classes=num_classes).cuda() for i in range(args.K)]
...
def train_one_epoch(client_id, epoch, global_model, model, prev_model, train_loader):
    start_epoch = time.time()

    losses = libs.AverageMeter()
    losses_cl = libs.AverageMeter()
    sim_global = libs.AverageMeter()
    sim_prev = libs.AverageMeter()
    top1 = libs.AverageMeter()
    top5 = libs.AverageMeter()
    ...
    for local_epoch in range(args.local_epochs):
        for i, (X, y) in enumerate(train_loader):
            ...
            _, feature_global = global_model(X)

            logits = sim(feature, feature_global).reshape(-1,1)
            targets = torch.zeros(X.shape[0]).long().cuda()

            sim_global.update(logits.mean().data.item(), X.shape[0])

            if epoch > 0: 
                _, feature_prev = prev_model(X)
                logits_prev = sim(feature, feature_prev).reshape(-1,1)
                sim_prev.update(logits_prev.mean().data.item(), X.shape[0])
                logits = torch.cat((logits, logits_prev), dim=1)

            loss_cl = args.mu * loss_fn(logits / args.temp, targets)
            loss = loss_fn(y_pred, y)

            prec1, prec5 = libs.clf_accuracy(y_pred.data, y, topk = (1, 5))
            losses.update(loss.data.item(), X.shape[0])
            losses_cl.update(loss_cl.data.item(), X.shape[0])
            top1.update(prec1.data.item(), X.size(0))
            top5.update(prec5.data.item(), X.size(0))
    ...
    writer.add_scalar(f'loss_cl_{client_id}', losses_cl.avg, epoch)
    writer.add_scalar(f'sim_global_{client_id}', sim_global.avg, epoch)
    writer.add_scalar(f'sim_prev_{client_id}', sim_prev.avg, epoch)
...
for epoch in range(args.epochs):
    ...
    # training
    for i in range(args.K):
        client_w = train_one_epoch(i, epoch, global_model, models[i], prev_models[i], train_loaders[i])
        prev_models[i].load_state_dict(client_w)
        for key in global_w_next:
